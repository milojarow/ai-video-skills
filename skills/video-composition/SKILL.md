...
| Hand-written ffmpeg calls: keeping a long filter graph in a file (`-/filter_complex`, which replaced `-filter_complex_script` in ffmpeg 9), the quoting error that lies about its cause, and measuring one channel of a stereo mix (`pan` + `astats`) to prove `amerge` did not swap the two sources | [FFMPEG-CLI.md](FFMPEG-CLI.md) |
